...
def test_str_method(test_file):
    receipt = Receipts(test_file)
    assert receipt.__str__() == "Receipts for the file: '2024-04.csv'"


def test_init_with_invalid_file_path():
    with pytest.raises(FileNotFoundError):
        receipt = Receipts("some_file_path")


def test_check_file_structure_ok(test_file):
    receipt = Receipts(test_file)
    assert receipt.check_file_structure() is None


def test_check_file_structure_invalid_structure(test_file_invalid):
    receipt = Receipts(test_file_invalid)
    with pytest.raises(FileStructureError) as exc_info:
        receipt.check_file_structure()
...
def test_original_dataframe_ok(test_file):
    receipt = Receipts(test_file)
    actual_df = receipt.original_dataframe()
...
def test_generate_receipt_report_ok(test_file):
    receipt = Receipts(test_file)
    actual_df = receipt.generate_receipt_report()
...
def test_generate_receipt_report_short_ok(test_file):
    receipt = Receipts(test_file)
    actual_df = receipt.generate_receipt_report_short()
    exp_df = pd.read_excel(
        "receipts/tests/test_data/solutions.xlsx", sheet_name="2024-04_full"
    )
    exp_df.drop(
...
def test_convert_receipt_report_to_excel_file_exist(test_file):
    receipt = Receipts(test_file)
    new_file_path = "receipts/tests/temp_file.xlsx"
    receipt.convert_receipt_report_to_excel("receipts/tests/temp_file.xlsx")
...
def test_convert_receipt_report_to_excel_sheet_with_final_report(test_file):
    receipt = Receipts(test_file)
    new_file_path = "receipts/tests/temp_file.xlsx"
    ...
    receipt.convert_receipt_report_to_excel(new_file_path)
...
def test_convert_receipt_report_to_excel_sheet_with_dictionary(test_file):
    receipt = Receipts(test_file)
    new_file_path = "receipts/tests/temp_file.xlsx"
    receipt.convert_receipt_report_to_excel(new_file_path)

    data = [(k, v) for k, v in receipt.RECEIPT_HEADERS.items()]
